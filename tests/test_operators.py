...
@pytest.mark.task0_2
@given(small_floats, small_floats, small_floats)
def test_commutative(a: float, b: float, c: float) -> None:
    """
    Ensures Commutative property of multiplication
    z*(x*y) == (z*x)*y
    """
    assert_close(mul(c, mul(a, b)), mul(mul(c, a), b))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    for el in ls2:
        acc += el
    assert_close(sum(ls1) + sum(ls2), acc)
